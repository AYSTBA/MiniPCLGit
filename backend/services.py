...
def do_full_backup(src_root, save_root, selected_items, selected_save, note=""):
    success = []
    failed = []
    backup_root = os.path.join(os.path.expanduser("~"), BACKUP_FOLDER_NAME)
    ver = os.path.basename(os.path.dirname(save_root)) if "versions" in save_root else "global"
    time_str = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    zip_name = f"备份_{time_str}_{note}.zip"
    target_zip = os.path.join(backup_root, ver, selected_save, zip_name)
...
def get_backup_history(version, save_name):
    backup_root = os.path.join(os.path.expanduser("~"), BACKUP_FOLDER_NAME)
    backup_dir = os.path.join(backup_root, version, save_name)
    history = []
    if os.path.isdir(backup_dir):
        for f in sorted(os.listdir(backup_dir), reverse=True):
            if f.endswith(".zip"):
                ...
                history.append({"name": display, "path": path})
    return history
